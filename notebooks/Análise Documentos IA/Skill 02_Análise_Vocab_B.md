...
A implementação computacional desta skill — extração, tratamento do vocabulário e cálculo de similaridade entre os documentos — é realizada no notebook `analise_geral.ipynb`, na pasta "Análise Conjunta". A distinção entre as duas bases vocabulares definida no item 5, abaixo, é obrigatória em toda execução desse notebook, e em qualquer outro artefato de comparação conjunta produzido sob esta skill.
...
### 5. Duas bases vocabulares distintas — nunca confundir uma com a outra

Esta skill opera sobre **duas bases vocabulares diferentes**, cada uma reservada a um tipo de visualização específico. Confundir as duas — em particular, aplicar a base (b) onde a base (a) é exigida — é o erro metodológico mais grave que esta skill precisa prevenir, porque distorce silenciosamente qualquer medida de similaridade sem que isso seja visualmente óbvio no gráfico final.

**(a) Vocabulário completo pós-etapa 3 — base obrigatória para similaridade e proximidade entre documentos.** É o conjunto integral de termos de cada documento após as etapas 1-3 (idioma identificado, extração integral, remoção de termos não semânticos, normalização/agrupamento de variantes e exclusão de identificadores idiossincráticos de nacionalidade/instituição), **sem qualquer corte por rank ou contagem fixa**. Esta é a **única** base vocabular válida para:
- o cálculo de similaridade entre documentos (item 6, abaixo);
- qualquer matriz de similaridade, dendrograma, ou visualização de proximidade/distância entre documentos;
- toda análise comparada executada em `analise_geral.ipynb` que envolva medir o quanto os documentos se aproximam ou se distanciam entre si.

Truncar essa base por rank (ex.: Top 50) antes de um cálculo de similaridade é metodologicamente inválido, por três razões que devem ser sempre observadas e, se for o caso, reportadas como risco evitado:
1. **Zeros artificiais assimétricos** — um termo na posição 51 de um documento (presente, com frequência relativa real) seria zerado, enquanto o mesmo termo na posição 40 de outro documento manteria seu valor, distorcendo o cosseno por um artefato do corte, não por diferença textual real entre os documentos.
2. **Viés a favor do vocabulário genérico compartilhado** — termos de altíssima frequência (ex.: "AI", "policy", "innovation") tendem a dominar o Top 50 de todos os documentos do mesmo gênero textual, enquanto o vocabulário de frequência mais baixa mas distintivo de cada documento — o que de fato carrega poder discriminante entre eles — fica de fora do corte. Isso infla artificialmente a similaridade entre todos os pares e esconde divergências reais entre os planos nacionais.
3. **Cobertura desigual entre documentos** — a fração da massa total de tokens coberta por um Top 50 fixo varia conforme a riqueza lexical de cada documento (um documento com vocabulário mais diverso tem seu Top 50 cobrindo uma fatia menor do total do que um documento mais repetitivo), o que introduz uma assimetria adicional que a normalização por tamanho do item 4 não resolve.

**(b) Corte de Top 50 — exclusivo para visualizações de frequência/ranking por documento.** Definir e declarar explicitamente o critério utilizado para selecionar quais termos entram em visualizações que exibem os termos mais frequentes/proeminentes de cada documento individualmente (ex.: gráfico de barras do vocabulário mais frequente de cada país, tabela comparativa lado a lado dos termos mais emblemáticos de cada plano). O critério de corte padrão desta skill, herdado da Skill 02_Análise_Vocab_A, é o **Top 50** termos por documento (podendo um recorte de Top 25 ser mantido como visão adicional dentro do mesmo Top 50). O mesmo critério deve ser aplicado de forma **uniforme a todos os documentos comparados** — usar critérios de corte diferentes por documento introduziria um viés na comparação. Assim como na análise individual, o corte de Top 50 só é válido se acompanhado da varredura ampliada de normalização morfológica exigida na Skill 02_Análise_Vocab_A (item 6) em cada um dos documentos comparados. Esta base (b) **nunca** deve alimentar um cálculo de similaridade — seu único uso legítimo é como recorte de exibição de frequência por documento.

Sempre que uma visualização for construída, deve-se identificar explicitamente qual das duas bases (a ou b) foi utilizada e por quê, conforme o tipo de gráfico demandado (item 9, abaixo).

### 6. Métrica de comparação/similaridade

**Método padrão de mensuração de similaridade entre documentos.** A similaridade de vocabulário entre dois ou mais documentos — incluindo toda comparação executada em `analise_geral.ipynb` — é sempre calculada sobre o **vocabulário completo pós-etapa 3** (item 5-a), **nunca** sobre o subconjunto de Top 50 termos (item 5-b). Essa base é representada como um **vetor de frequência relativa** — a proporção de cada termo em relação ao total de tokens de conteúdo do próprio documento, nunca a contagem absoluta (consistente com a exigência de normalização relativa do item 4 desta skill). Cada documento é assim representado por um vetor no mesmo espaço de termos — a união dos vocabulários completos e normalizados de todos os documentos incluídos na comparação —, e a similaridade entre um par de documentos é calculada sobre esses vetores (ex.: similaridade de cosseno), produzindo um valor único e comparável para cada par. Esse é o método de referência sempre que a demanda do usuário for "similaridade entre documentos" em termos gerais, e deve ser declarado explicitamente como tal sempre que utilizado — incluindo a confirmação explícita de que **nenhum corte por rank foi aplicado** antes do cálculo.

Isso não substitui outras formas de análise de vocabulário/temática que o usuário venha a solicitar de modo complementar ou alternativo (ex.: sobreposição de conjuntos de termos mais frequentes, comparação de ranking, proximidade de frequência relativa em torno de um tema específico) — não há um tipo de gráfico fixo ao qual esta skill esteja presa. Qualquer que seja a métrica efetivamente utilizada em uma visualização, ela deve ser declarada e justificada explicitamente, nunca aplicada de forma implícita, e deve indicar sobre qual das duas bases vocabulares (item 5) foi calculada.

### 6.1. Sequência estruturada da comparação

Toda comparação desta seção opera sobre a base vocabular completa (item 5-a), consistente com o método de similaridade do item 6. A comparação entre os documentos deve seguir uma sequência estruturada, do mais específico para o mais amplo, em vez de comparar todos os pares de documentos de forma desordenada:
...
- A métrica de similaridade utilizada (item 6 — vetor de frequência relativa calculado sobre o **vocabulário completo pós-etapa 3**, item 5-a, sem corte por rank — e método de cálculo, ex. similaridade de cosseno) e a sequência de comparação seguida (item 6.1), quando a demanda envolver mensuração de similaridade entre os documentos;
- Qual das duas bases vocabulares do item 5 foi utilizada em cada visualização produzida (vocabulário completo, item 5-a, ou corte de Top 50, item 5-b) e, quando a base utilizada for a (b), o critério de corte aplicado.
...
- Método de mensuração de similaridade utilizado (item 6 — vetor de frequência relativa sobre o **vocabulário completo pós-etapa 3**, item 5-a, sem corte por rank) e, quando aplicável, os resultados organizados segundo a sequência estruturada de comparação (item 6.1: coerência interna por bloco, comparação entre blocos, posicionamento do PBIA);
- Método de normalização relativa aplicado (item 4);
- Base vocabular utilizada em cada visualização (vocabulário completo, item 5-a, para similaridade/proximidade; Top 50 por documento, item 5-b, para visualizações de frequência) e o critério de corte aplicado nesta última;
...
Construir a visualização gráfica solicitada pelo usuário sempre a partir da base vocabular correta para o tipo de gráfico (item 5):
- **Vocabulário completo pós-etapa 3 (item 5-a)** para qualquer visualização de similaridade, proximidade ou distância entre documentos (ex.: matriz/heatmap de similaridade, dendrograma, mapa de proximidade) — sempre o caso em `analise_geral.ipynb` quando a demanda envolver medir o quanto os documentos se aproximam ou se distanciam entre si;
- **Corte de Top 50 (item 5-b)** apenas para visualizações de frequência/ranking de termos por documento (ex.: gráfico de barras dos termos mais frequentes de cada país, tabela lado a lado dos termos emblemáticos de cada plano).

Não há tipo de gráfico fixo predefinido além dessa distinção de base vocabular — o formato depende exclusivamente do que for demandado, desde que represente adequadamente a natureza relativa (e não absoluta) da comparação (item 4) e utilize a base vocabular correta (item 5) para o tipo de visualização escolhido.
...
- A metodologia aplicada (idiomas envolvidos, documento central, tratamento de bigramas/hífens, desambiguação contextual, exclusão de identificadores idiossincráticos nacionais/institucionais, método de normalização relativa, **base vocabular utilizada em cada visualização** — vocabulário completo, item 5-a, para similaridade/proximidade, ou Top 50, item 5-b, para frequência por documento —, critério de corte quando aplicável, métrica de comparação e, quando aplicável, a sequência estruturada de comparação seguida — item 6.1);
...
Toda medida de similaridade entre os documentos — em particular as produzidas em `analise_geral.ipynb` — deve refletir o **vocabulário completo pós-etapa 3** de cada documento (item 5-a), nunca um recorte por rank, garantindo a máxima fidelidade possível entre o gráfico produzido e o conteúdo real dos documentos comparados. O corte de Top 50 (item 5-b) permanece reservado, exclusivamente, às visualizações de frequência/ranking por documento.
